...
impl PcrOp for TdxRTMR {
    fn read_pcr(&self, reg: u8) -> PcrValue {
        ...
        if let Ok(wapper) = tdxguest::kernel_handle_get_report(req) {
            ...
            let target_offest = tdreport_info_offest
                + tdreport_info_rtmrs_offest
                + reg as usize * TDX_EXTEND_RTMR_DATA_LEN;
            let target_rtmr =
                &wapper.tdx_report[target_offest..target_offest + TDX_EXTEND_RTMR_DATA_LEN];
    ...
    fn extend_pcr(&self, reg: u8, data: PcrValue) {
        ...
        let _ = tdxguest::kernel_handle_extend_rtmr(ExtendRtmrWapper {
            data: tmp,
            index: reg,
        });
    }
