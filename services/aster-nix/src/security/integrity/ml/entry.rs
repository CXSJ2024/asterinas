...
use crate::security::integrity::ima::ima_hash::{IMAAlogrithm, IMAHash, VecU8};
...
impl Display for MeasurementEntry {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(
            f,
            "pcr:{}\t template-hash:{:x?}\t filedata-hash:{:x?}\t filename-hint:{}]",
            self.pcr, self.template_hash, self.filedata_hash, self.filename_hint
        )
    }
}
...
impl Into<String> for MeasurementEntry {
    fn into(self) -> String {
        ...
        format!(
            "{} {} {} {} {}\n",
            self.pcr, template_str, template_type, content_str, self.filename_hint
        )
    }
}
